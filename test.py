...
def scs(N, l, r, Field, barrier, verific, together, A, B, m, n, p, q):
...
if __name__ == "__main__":
	...
	#parser.add_argument('--r', type=int, help='divide A on K')
	parser.add_argument('--N', type=int, help='divide B on L')
...
#	A = np.matrix([[0]*(n) for i in range(m)])  
#	B = np.matrix([[0]*(n) for i in range(p)])
	...
	if MPI.COMM_WORLD.rank == 0:
	    print A 
	    print B
	scs(N, l, r, Field, barrier, verific, together, A, B, m, n, p, 0)
