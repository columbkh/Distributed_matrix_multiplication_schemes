import same_field_choose_ra_rb as scsa
import gscsa_same_field as gscsa
import uscsa_same_field as uscsa

def do_multiple_tests(r_a, r_b, l, field, q, m, n, p, verific, together, number, coeff):
    scsa.write_title_to_octavemnp(q, field, m, n, p, number, coeff)
    scsa.set_communicators(r_a, r_b, l, field)
    ...
    for i in range(number):
        if MPI.COMM_WORLD.rank == 0:
            ...
            print "p: ", p
            print "--------------------------------------------------------------------------"
        scsa.do_test(r_a, r_b, l, field, q, m, n, p, verific, together)
        uscsa.do_test(r_a, r_b, l, field, q, m, n, p, verific, together)
        gscsa.do_test(r_a, r_b, l, field, q, m, n, p, verific, together)
