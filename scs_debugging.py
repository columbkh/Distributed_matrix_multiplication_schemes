...
def scs_m(N, l, r, Field, barrier, verific, together, A, B, m, n, p):
	#m = 500
	#n = 500
	#p = 500
...
#		p = (p // r) * r
	...
	if prev_comm.rank == 0:  # Master
	    ...
	    #A = np.matrix(np.random.random_integers(0, 0, (m, n)))
	    #B = np.matrix(np.random.random_integers(0, 0, (p, n)))

           # A = np.matrix([[1]*(n) for i in range(m)])
           # B = np.matrix([[1]*(n) for i in range(p)])  
...
if __name__ == "__main__":
   ...
   #  parser.add_argument('--i', type=int, help='i')
     ...
     if MPI.COMM_WORLD.rank == 0:
          scs_m(N, l, r, Field, barrier, verific, together, A, B, m, n, p)
     else:
