...
def encode_An(lpart, i_plus, A, Field, l, r, Zik):
	return [lpart[i] * ((A + sum([(pow(i_plus[i], k, Field) * Zik[i][k-1]) % Field for k in range(1, l+1)])) % Field) for i in range(r)]
...
def encode_Bn(Bn, i_plus, Field, l, r, N, Zik):
	return [(Bn[i] + sum([(pow(i_plus[i], k, Field) * Zik[i][k-1]) % Field for k in range(1, l+1)])) % Field for i in range(r)]
	
def encode_B(Bn, i_plus_an, Field, l, r, N):
	Zik = [[np.matrix(np.random.random_integers(0, 0, (Bn[0].shape[0], Bn[0].shape[1]))) for k in range(l)] for i in range(r)]
...
def getAenc(Ap, Ka, N, F, l, r_a, x):
	return [sum([Ap[j] * pow(x[i], j, F) for j in range(r_a)]) % F + sum([Ka[k] * pow(x[i], k+r_a, F) for k in range(l)]) % F for i in range(N)]

def getBenc(Bp, Kb, N, F, l, r_a, r_b, x):
	return [sum([Bp[j] * pow(x[i], j*(r_a+l), F) for j in range(r_b)]) % F + sum([Kb[k] * pow(x[i], k+r_a+(r_b-1)*(r_a+l), F) for k in range(l)]) % F for i in range(N)]
